...
class Exercise(object):
    ...
    def __init__(self, exercise, link, sets, reps):
...
def get_weights():
    for item in Post_workout.objects.all():
        if (item.equipment != 'Other' and item.equipment != 'Body Only' and
            ...
            and item.rating > 8.5):
            workout = Workout(item.title, item.rating, item.videolink, item.musclegroup, item.equipment, item.purpose)
